...
= 0.1.471 =
* Adds the translated archive page number to Rank Math SEO titles on paginated translated blog archive pages.

= 0.1.470 =
* Uses the actual translated blog archive page number when generating Rank Math canonical URLs for paginated archive pages.
...
* Centralizes public agent session schema and provenance shaping so protected abilities, reservations, heartbeat, and review checks use the same identity contract.

= 0.1.467 =
* Removes the last stale heartbeat schema alias text so agent_session_id is the only public heartbeat session identity.
...
* Adds a shared request-local analysis cache for repeated Gutenberg and editorial/content-integrity checks.
* Makes workflow-status compact by default with an explicit full detail mode.

= 0.1.464 =
* Makes the language-list ability compact by default, with an explicit full detail mode for runtime text and quality profile payloads.
* Moves workflow obligations onto the shared work-item read model so dashboard and coordinator calls avoid rebuilding heavy translation rows.
* Adds request-local caching for Gutenberg guardrails and source editorial design validation.
...
* Removes packaged language-quality JSON policy so language-specific QA policy lives in runtime profiles or audited rule events.

= 0.1.460 =
* Removes route and path seed data from packaged language JSON files and rejects future packaged route fields.
* Stops requiring packaged 404 route lists in language-file health checks.

= 0.1.459 =
* Computes translated post permalinks from the live translated blog archive route so stale stored localized paths cannot keep archive item links on old packaged blog defaults.

= 0.1.458 =
* Removes packaged blog path defaults from language files so live WordPress posts-page translations remain the source of truth for blog archive routes.

= 0.1.457 =
* Uses published translated posts-page permalinks for blog archive presentation language links before falling back to packaged default blog paths.

= 0.1.456 =
* Adds compact source-design queue evidence for featured-image media contract failures.

= 0.1.455 =
* Carries editorial vertical-centering design contract metrics into source-design repair queue payloads so layout improvements remain discoverable as queued work.

= 0.1.454 =
* Adds content-integrity repair queue support for source and translated Gutenberg content, including Rank Math FAQ blocks with unparseable question attributes or disallowed FAQ item links.

= 0.1.453 =
* Prioritizes actual invalid source-design posts before queue limits are applied so worker-sized scans see repair work.

= 0.1.452 =
* Scans published original posts as source-design work candidates so legacy source articles can enter the repair queue without translation-row hardcoding.

= 0.1.451 =
* Adds a first-class work item catalog for queue and heartbeat selection so source-design repair can be claimed before downstream translation reprojection work.

= 0.1.450 =
* Matches translated blog archive card spacing to the native GeneratePress blog loop so localized posts do not crowd the archive hero.

= 0.1.449 =
* Enforces translated page hierarchy from the translated source parent so a caller-supplied parent path or ID cannot move a child page into the wrong localized hierarchy.

= 0.1.448 =
* Uses a compact source-design target read model for migration and reprojection so exact translation repairs do not expand full workflow review payloads first.

= 0.1.447 =
* Passes session identity input into source-design migration and reprojection claim gates so bound worker reservations are recognized correctly.
...
= 0.1.444 =
* Adds source-link coverage diagnostics to source-structure guardrail failures so workers can fix the missing source links instead of guessing from raw link counts.

= 0.1.443 =
* Refines source-structure link guardrails so translations preserve source internal link coverage and action links without blocking on harmless repeated-anchor count drift.

= 0.1.442 =
* Regenerates Rank Math FAQ saved markup from block attributes during storage normalization and makes source-design reprojection resave repairable stored markup drift.

= 0.1.441 =
* Preserves translation fitness guardrail module details so dimension-filtered scans can attribute source-fidelity issues correctly.

= 0.1.440 =
* Fixes translation fitness dimension filtering so source-fidelity scans include source editorial design-gate failures.

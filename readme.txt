...
* Project language menus explicitly when available translations or menu labels require a navigation update.
...
= 0.1.686 =
* Reopen an exact live-verified published Source Rewrite Artifact for replacement independent Quality without changing page bytes or discarding immutable review history.

= 0.1.685 =
* Serialize Source Rewrite and Translation Quality behind one exact-artifact, installation-wide lease; offer the next Quality claim only after terminal release or fail-closed expiry recovery.

= 0.1.684 =
* Let the first translated static front page establish the native language-prefix root; fail dependent page jobs closed until that root exists.

= 0.1.683 =
* Keep Source Inventory generation materialization and activation under one owner lease so concurrent terminal rebuild resumes cannot invalidate the active queue.

= 0.1.682 =
* Remove caller-controlled and implicit menu synchronization from translation publication; keep receipt-bound `activate-public-header-projection` as the separate explicit coordinator operation.

= 0.1.681 =
* Keep Source Rewrite live verification exact across content-owned templates, semantic title/excerpt hosts, and Cloudflare-protected mailto actions.
...
= 0.1.678 =
* Supply exact authorized Source and Translation preview content to the reusable GenerateBlocks request-content projection Interface.

= 0.1.677 =
* Keep invalid or expired preview capabilities on a non-canonical, non-cacheable 404 instead of forwarding their token through a redirect.

= 0.1.676 =
* Treat WordPress's equal `page_id` and `p` aliases as one exact preview host while rejecting genuinely conflicting IDs.

= 0.1.675 =
* Read staged-preview authority from the exact query invoking the projection filter instead of timing-dependent global query state.

= 0.1.674 =
* Project staged bytes only when the normalized main query and its original query-ID request resolve to the same exact WordPress object.

= 0.1.673 =
* Bind staged-preview response policy to the original query-ID request before WordPress normalizes the main query for canonical redirects.

= 0.1.672 =
* Preserve exact authorized staged-preview routes against WordPress canonical redirects so independent Quality can inspect the claimed artifact.
* Allow explicit rediscovery to create one bounded successor cycle after a Source Rewrite Job reaches terminal exhaustion.

= 0.1.671 =
* Keep ordinary WordPress requests safe when staged-preview filters run before the main query exists.
...
= 0.1.668 =
* Centralize staged-preview page-cache prevention in the shared capability module while retaining the established WordPress cache interoperability signal with an explicit standards justification.
...
* Block ordinary public source-copy changes and downstream translation while a current source rewrite lacks verified approval.
* Prime fresh writing, translation, and Quality runs with research-led effectiveness guidance and a bounded source-scoped policy Adapter, without site-specific copy in the public plugin.
...
* Apply source-design fragment-role guardrails to translated pages as well as posts.

= 0.1.663 =
* Derive a missing legacy parent path from the exact signed translated-parent identity after language and hierarchy validation.
* Keep every non-empty signed parent path as an exact fail-closed route assertion.

= 0.1.662 =
* Publish pre-0.1.661 new-page artifacts by deriving their one valid path from the signed parent and slug.
* Keep route compatibility fail-closed when the signed parent identity or parent path has changed.

= 0.1.661 =
* Derive each new translated page path from its resolved WordPress parent and localized slug before staging.
* Apply and verify the exact signed page route before accepting a newly created translation surface.

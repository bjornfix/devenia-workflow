...
- Keeps Source Inventory generation materialization and activation under one owner lease, with idempotent terminal resumes that cannot rewrite an already-active generation or invalidate its queue digest.
...
- Keeps Source Rewrite live verification exact across content-owned templates, semantic title/excerpt hosts, and Cloudflare-protected mailto actions.
...
- Restores claim-bound Translation previews for the lifecycle's actual `quality_pending` state while rejecting malformed, cross-Job, wrong-generation, wrong-contract, and expired bindings.

### 0.1.679

- Verifies equivalent homepage URLs and reversible Cloudflare-protected mailto actions while keeping wrong destinations release-blocking.

### 0.1.678

- Supplies exact authorized Source and Translation preview content to the reusable GP-MCP request-content projection Interface without owning GenerateBlocks CSS or cache behavior.

### 0.1.677

- Keeps invalid or expired preview capabilities on a non-canonical, non-cacheable 404 instead of forwarding their token through a redirect.

### 0.1.676

- Treats WordPress's equal `page_id` and `p` aliases as one exact preview host while rejecting genuinely conflicting IDs.

### 0.1.675

- Reads staged-preview authority from the exact query invoking the projection filter instead of timing-dependent global query state.

### 0.1.674

- Projects staged bytes only when the normalized main query and its original query-ID request resolve to the same exact WordPress object.

### 0.1.673

- Binds staged-preview response policy to the original query-ID request before WordPress normalizes the main query for canonical redirects.
...
- Keeps ordinary WordPress requests safe when staged-preview filters run before the main query exists.

### 0.1.670

- Returns staged associative metadata through WordPress' native outer value-list contract, eliminating preview warnings while preserving exact route and presentation values.

...
const wrongIndexRuntimeContractPasses = (source) => {
	...
	return adapterStart > 0
		...
		&& /! \$wrong_index_injected[\s\S]*1 !== \$wrong_index_commit_calls[\s\S]*\$wrong_index_commit_receipt\['success'\][\s\S]*! empty\( \$wrong_index_rows_missing \)[\s\S]*empty\( \$wrong_index_publish\['success'\] \)[\s\S]*forward_publication_applied[\s\S]*published[\s\S]*published_unverified[\s\S]*! empty\( \$wrong_index_nested_failure \)[\s\S]*translation_index'\]\['success[\s\S]*\$wrong_index_rows_before !== \$wrong_index_rows_after[\s\S]*\$wrong_index_header_before[\s\S]*\$wrong_index_menu_before[\s\S]*\$wrong_index_menu_inventory_before/.test(source);
};
assert.equal(wrongIndexRuntimeContractPasses(wrongIndexRuntime), true, "ordinary Translation Job publication must ignore an unrelated post-COMMIT Index row loss, preserve every menu surface, and verify only its own queue row");

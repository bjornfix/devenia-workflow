...
const uninstall = readFileSync(new URL("../uninstall.php", import.meta.url), "utf8");
const abilityCatalogue = readFileSync(new URL("../includes/trait-ability-catalogue.php", import.meta.url), "utf8");
const abilityPlatform = readFileSync(new URL("../includes/trait-ability-platform.php", import.meta.url), "utf8");
...
const firstEnrollmentCleanupRuntime = readFileSync(new URL("./check-public-header-first-enrollment-cleanup-runtime.php", import.meta.url), "utf8");
...
const syncStart = plugin.indexOf("private static function stage_language_menu_projection");
const syncEnd = plugin.indexOf("private static function existing_menu_label_map", syncStart);
assert.ok(syncStart > 0 && syncEnd > syncStart, "stage_language_menu_projection must remain a bounded implementation");
...
const publicHeaderSyncStart = publication.indexOf("private static function activate_public_header_projection");
const publicHeaderSyncEnd = publication.indexOf("private static function configured_public_header_languages", publicHeaderSyncStart);
...
assert.match(publication, /publish_localized_presentation[\s\S]*apply_translation_publish_transition[\s\S]*devenia_workflow_frontend_cache_invalidation_result/);
assert.doesNotMatch(publication.slice(publication.indexOf("private static function publish_localized_presentation")), /refresh_public_header_projection_for_publication|activate_public_header_projection|sync_menu|['\"]menu['\"]\s*=>/, "content publication must remain content-only");
...
assert.doesNotMatch(publication.slice(publication.indexOf("private static function publish_localized_presentation"), publication.indexOf("private static function rollback_localized_menu_projection")), /stage_language_menu_projection\s*\(/, "normal publication must not activate one language directly");
...
assert.match(publication, /frontend_cache_adapter_missing[\s\S]*commit_reconciliation[\s\S]*frontend_cache_invalidation_failed[\s\S]*commit_reconciliation/, "every synchronous content post-commit failure must preserve commit reconciliation and the exact mutation receipt");
...
assert.doesNotMatch(publication, /stage_public_header_manifest_for_publication|refresh_public_header_projection_for_publication/, "ordinary publication must have no Public Header staging or activation helper");
...
assert.match(enrollmentInterface, /stage_first_public_header_enrollment_transaction[\s\S]*'activation_receipt'\s*=> \(string\) \( \$stage\['activation_receipt'\]/, "first enrollment must return its transaction-owned pending receipt for a separate activation call");
assert.doesNotMatch(enrollmentInterface, /\$input\['activate'\]|activate_public_header_projection/, "first enrollment must stage only and never activate implicitly");
...
assert.match(plugin, /public_header_activation_input_schema[\s\S]*'required'\s*=> array\( 'activation_receipt' \)[\s\S]*\^phact_\[a-f0-9\]\{48\}\$/, "the explicit Public Header activation Interface must require the opaque activation receipt");
...
assert.match(publication, /build_public_header_transition[\s\S]*previous_menu_surface_revision[\s\S]*public_header_navigation_snapshot_from_menu[\s\S]*'expected_navigation'[\s\S]*'rollback'/, "the durable transition must bind the prior menu receipts and exact rollback navigation before activation");
assert.match(publication, /start_public_header_transition_rollback[\s\S]*rollback_target_state[\s\S]*rollback_invalidation_pending[\s\S]*replace_public_header_state_transaction/, "rollback must restore only the transition-bound reader state and remain verification pending");
assert.doesNotMatch(publication, /public_header_failure_after_activation|public_header_rollback_projection_receipts/, "the retired synchronous all-language rollback helpers must not remain as a second implementation");
...
assert.match(publication, /activate_public_header_projection[\s\S]*validate_public_header_authority_receipts[\s\S]*devenia_workflow_public_header_authority_before_final_revalidation[\s\S]*public_header_authority_changed_before_activation/, "authority receipts must be validated before and after complete-set staging");
assert.match(publication, /enroll_public_header_from_existing_menus[\s\S]*public_header_relation_receipts_for_manifest[\s\S]*stage_first_public_header_enrollment_transaction/, "explicit first enrollment must mint fresh relation receipts before pending state exists");
...
assert.match(publicHeaderRuntime, /canonical_relation_post_ids[\s\S]*source_home[\s\S]*translation_routes[\s\S]*raw_relation_post_surface\( \$canonical_relation_post_ids \)/, "raw relation-state oracles must initialize canonical source and translated routes before race snapshots use them");
...
assert.match(runtime, /translation_job_publication_is_content_only/, "Translation Job runtime must report the content-only publication boundary");
...
const failedProjectionContractPasses = (source) => {
	...
	return /stage_language_menu_projection', array\([\s\S]*'language' => \$language[\s\S]*'manifest' => \$runtime_pending_manifest/.test(bounded)
...
assert.match(translationJobRuntimeSuite, /check-staged-preview-capability-runtime\.php[\s\S]*canonical-redirect runtime passed[\s\S]*check-new-page-localized-path-contract\.php[\s\S]*DEVENIA_WORKFLOW_RUNTIME_SCOPE=new-page-route[\s\S]*check-translation-job-runtime\.php[\s\S]*legacy_new_page_localized_path_derived_from_signed_route_inputs[\s\S]*translation-job-change-scoped/, "route/publication releases must prove the shared canonical-preview Adapter and have a bounded Translation Job gate instead of exercising the unrelated Public Header stress suite on shared dev hosting");
assert.match(translationJobRuntimeSuite, /check-new-translation-staged-preview-wordpress-runtime\.php[\s\S]*target_runtime_language[\s\S]*zero_source_mutation[\s\S]*existing_translation_preview/, "the bounded Translation Job gate must execute first- and existing-translation staged preview runtime proof");
...
const queryIdentityLinkAuthorityPasses = (pluginSource, readModelSource, runtimeSource) => {
	...
	return /array_merge\([\s\S]*\$source_variants\[ \$source_id \][\s\S]*content_shortlink_variants\( \(int\) \$source_id \)/.test(indexedMap)
		...
		&& /\$source_is_root[\s\S]*\$source_url\s*= \$source_is_root \? home_url\( '\/' \)[\s\S]*get_permalink\( \$source_id \)[\s\S]*\$source_id === self::wordpress_content_query_id_from_parts/.test(frontendRows)
		&& /get_permalink\( \$translation_id \)[\s\S]*frontend_route_path_from_url\( \$target_url \)/.test(frontendRows)
		&& !/get_post\(/.test(frontendRows)
...
const collapsedEmptySourcePath = indexReadModel.replace("$source_url     = $source_is_root ? home_url( '/' ) : ( '' === $source_path ? '' : home_url( '/' . $source_path . '/' ) );", "$source_url     = home_url( '/' . trim( $source_path, '/' ) . '/' );");
...
assert.match(enrollmentInterface, /pre_enrollment_public_header_recovery_snapshot[\s\S]*'pre_enrollment_recovery'\s*=>\s*\$recovery[\s\S]*stage_first_public_header_enrollment_transaction/, "first enrollment must capture and bind pre-intake reader evidence before it stages pending authority");
assert.match(publicHeaderSync, /\$pending\['pre_enrollment_recovery'\][\s\S]*\$recovery_revision[\s\S]*\$input\['pre_enrollment_recovery'\]/, "the sole explicit activation Interface must validate and consume first-enrollment rollback evidence from its receipt-bound pending authority");
assert.match(publication, /unset\( \$active_manifest\['pre_enrollment_recovery'\] \)/, "ephemeral first-enrollment recovery evidence must never become active reader authority");
...
assert.match(publication, /public_header_database_state[\s\S]*SELECT option_name, option_value[\s\S]*maybe_unserialize[\s\S]*reconcile_public_header_state_commit_outcome[\s\S]*public_header_database_state[\s\S]*reconcile_first_public_header_enrollment_commit_outcome[\s\S]*public_header_database_state/, "both Public Header COMMIT reconciliation seams must classify durable raw option rows instead of persistent cache");
...
assert.match(publicHeaderSync, /! empty\( \$result\['activation_applied'\] \) \) \{ return \$result; \}[\s\S]*public_header_reader_state_unchanged[\s\S]*\$recovered = \$exact && ! empty\( \$cleanup\['success'\] \)/, "a proven-unapplied first-enrollment activation must terminate through exact state restoration and candidate cleanup without cache or live-reader work");
...
assert.match(publicHeaderRuntime, /\$runtime_batch_http[\s\S]*expected_navigation[\s\S]*devenia_workflow_frontend_cache_batch_adapter_result[\s\S]*verify_transition_languages/, "the state-machine runtime may replace only the external HTTP Adapter with receipt-bound navigation while the separate live gate owns real Requests transport proof");
assert.match(publicHeaderRuntime, /\$activate_a\s*=\s*\$call\( 'activate_public_header_projection'[\s\S]*\$activate_a_verification\s*=\s*\$verify_transition_languages\( \$stage_a_receipt \)[\s\S]*forward_verified[\s\S]*\$missing_active_label_authority/, "every completed activation fixture must reach explicit per-language verification before a later manifest operation can exercise its own contract");
assert.match(firstEnrollmentCleanupRuntime, /wp_get_ability\( 'devenia-workflow\/enroll-public-header-from-existing-menus' \)[\s\S]*wp_get_ability\( 'devenia-workflow\/activate-public-header-projection' \)[\s\S]*wp_get_ability\( 'devenia-workflow\/verify-public-header-projection' \)[\s\S]*check_permissions[\s\S]*timeout' => 2[\s\S]*is_wp_error\( \$invalid_verify_timeout \)[\s\S]*\$enrollment_ability->execute[\s\S]*\$activation_ability->execute[\s\S]*\$verification_ability->execute/, "the focused first-enrollment regression must cross registered permission, schema, and execute Interfaces for staging, explicit activation, and bounded verification");
assert.match(firstEnrollmentCleanupRuntime, /devenia_workflow_frontend_cache_batch_adapter_result[\s\S]*public_header_projection_rollback[\s\S]*forward_invalidation_pending[\s\S]*public_header_rollback_verification_pending[\s\S]*rollback_invalidation_pending[\s\S]*public_header_rollback_cache_invalidation_failed[\s\S]*rolled_back_verified[\s\S]*remaining_projection_ids[\s\S]*managed_before/, "the focused regression may replace only the external HTTP edge while proving explicit activation, retryable bounded rollback verification, and zero receipt-owned menu residue");
assert.doesNotMatch(firstEnrollmentCleanupRuntime, /delete_staged_public_header_projections|public_header_staged_cleanup_state_authority|public_header_state_excludes_staged_menu_ids/, "the focused regression must observe cleanup through the public operation instead of invoking cleanup implementation helpers");
assert.doesNotMatch(firstEnrollmentCleanupRuntime, /ReflectionMethod|guard_unapproved_source_rewrite_before_save|block_unready_source_post_publish_before_save|remove_filter\( 'wp_insert_post_data'/, "the focused public-ability regression must not bypass internal policy Interfaces");
assert.match(runtime, /\$runtime_batch_http[\s\S]*devenia_workflow_frontend_cache_batch_adapter_result[\s\S]*activate_public_header_projection[\s\S]*remove_filter\( 'devenia_workflow_frontend_cache_batch_adapter_result'/, "the exact Translation Job runtime must keep Public Header verification deterministic through the same whole-batch Adapter while retaining its separate single-request media fixture");
assert.doesNotMatch(runtime, /\$runtime_pending_manifest\s*=\s*get_option\( 'devenia_workflow_pending_public_header_manifest'[\s\S]*update_option\( 'devenia_workflow_public_header_manifest', \$runtime_pending_manifest/, "the Translation Job runtime must establish Public Header reader authority only through explicit activation, never by copying pending authority into the active manifest fixture");
...
assert.doesNotMatch(publication, /'passed' => true[\s\S]*self_referential_http_disabled/, "a skipped Public Header reader check must never be reported as passed");
assert.match(abilityCatalogue, /devenia-workflow\/verify-public-header-projection[\s\S]*public_header_verification_input_schema[\s\S]*run_ability_operation\( 'verify_public_header_projection'/, "Public Header live verification must be a separate registered root operation");
assert.match(abilityPlatform, /'verify_public_header_projection'\s*=>\s*'verify_public_header_projection'/, "the operation dispatcher must expose the Public Header verification Interface");
assert.match(plugin, /private static function public_header_verification_input_schema[\s\S]*activation_receipt[\s\S]*language[\s\S]*homepage and blog archive[\s\S]*additionalProperties'\s*=>\s*false/, "the Public Header verification Interface must bind one exact activation and one configured language per call");
assert.match(plugin, /OPTION_PUBLIC_HEADER_TRANSITION[\s\S]*ensure_public_header_transition_option[\s\S]*add_option\([\s\S]*'phase' => 'idle'[\s\S]*false \)/, "the transition Module must own one durable non-autoloaded lock row");
assert.match(publication, /replace_public_header_state_transaction[\s\S]*OPTION_PUBLIC_HEADER_TRANSITION[\s\S]*FOR UPDATE[\s\S]*expected_transition[\s\S]*replacement_transition[\s\S]*reconcile_public_header_state_commit_outcome/, "activation and its durable transition must share one locked transaction and commit reconciliation");
assert.match(publication, /SELECT option_id FROM \{\$wpdb->options\}[\s\S]*FOR UPDATE[\s\S]*false === \$locked_options[\s\S]*\$map = array\([\s\S]*wp_cache_delete\( \$key, 'options' \)[\s\S]*\$expected_exact[\s\S]*public_header_state_changed[\s\S]*atomic_create_option/, "the transaction must lock every currently present owned row, then enforce exact per-slot state CAS; missing rows remain protected by the unique option-name insert");
const activationCore = publication.slice(publication.indexOf("private static function activate_public_header_projection_core"), publication.indexOf("private static function configured_public_header_languages"));
assert.doesNotMatch(activationCore, /verify_public_header_projection_set|retire_previous_public_header_projection_set/, "activation must not verify or retire the prior set synchronously");
assert.match(activationCore, /activation_applied[\s\S]*verification_pending[\s\S]*needs_live_verification/, "activation must truthfully expose an applied but incomplete transition");
const headerVerify = publication.slice(publication.indexOf("private static function verify_public_header_projection( array $input"), publication.indexOf("private static function activate_public_header_projection_set"));
assert.match(headerVerify, /validate_public_header_transition[\s\S]*acquire_public_header_transition_lease[\s\S]*public_header_frontend_cache_response_set\( array\( \$language \), \$timeout, 1 \)[\s\S]*replace_public_header_transition/, "one verification call must validate authority, lease the transition, fetch exactly one language serially, and CAS its evidence");
assert.match(headerVerify, /evidence'\]\[ \$direction \]\[ \$language \][\s\S]*public_header_transition_evidence_complete/, "the verification operation must accumulate direction-specific evidence before terminal progression");
assert.match(publication, /acquire_public_header_transition_lease[\s\S]*lease_token[\s\S]*lease_expires_at[\s\S]*replace_public_header_transition/, "the transition Module must prevent overlapping self-fetch through one CAS lease");
assert.match(publication, /public_header_frontend_cache_response_set\( array \$languages, int \$timeout, \?int \$concurrency_limit = null \)[\s\S]*fetch_frontend_cache_surfaces\( \$requests, \$timeout, \$concurrency_limit \)/, "the existing Requests Adapter must accept an operation-owned concurrency cap");
assert.match(publication, /fetch_frontend_cache_surfaces\( array \$requests, int \$timeout, \?int \$concurrency_limit = null \)[\s\S]*null === \$concurrency_limit[\s\S]*PUBLIC_HEADER_REQUEST_CONCURRENCY_LIMIT/, "ordinary callers keep the bounded default while Public Header transition verification can force serial dispatch");
assert.match(publication, /activate_public_header_projection\( array \$input \)[\s\S]*public_header_transition_for_activation_receipt[\s\S]*activation_applied[\s\S]*needs_live_verification[\s\S]*validate_public_header_activation_receipt/, "an activation retry must reconcile its durable transition before looking for consumed pending authority");
assert.match(publication, /\$activation_applied = ! in_array\( \$phase, array\( 'rolled_back_verified', 'rollback_invalidation_pending', 'rollback_verifying', 'rollback_cleanup_pending', 'critical_conflict' \)[\s\S]*'success' => \$activation_applied[\s\S]*'activation_applied' => \$activation_applied/, "an idempotent activation retry must never report success while rollback or critical conflict owns the receipt");
assert.match(publication, /update_public_header_manifest\( array \$input \)[\s\S]*public_header_transition_is_nonterminal[\s\S]*public_header_transition_in_progress/, "new staging must fail closed while a transition owns reader authority");
assert.match(headerVerify, /public_header_transition_evidence_complete[\s\S]*finalize_public_header_transition_forward/, "the last complete forward receipt must enter terminal finalization");
assert.match(publication, /finalize_public_header_transition_forward[\s\S]*retire_previous_public_header_projection_set[\s\S]*forward_verified[\s\S]*replace_public_header_state_transaction[\s\S]*finalized/, "forward finalization must revalidate, retire, and commit the terminal transition");
assert.match(publication, /start_public_header_transition_rollback[\s\S]*rollback_invalidation_pending[\s\S]*finalize_public_header_transition_rollback[\s\S]*delete_staged_public_header_projections[\s\S]*rolled_back_verified/, "a forward failure must atomically restore reader state and delay candidate cleanup until complete rollback proof");
assert.match(headerVerify, /rollback_direction[\s\S]*verify_pre_enrollment_public_header_navigation[\s\S]*finalize_public_header_transition_rollback/, "the same resumable operation must perform bounded first-enrollment rollback proof");
assert.doesNotMatch(headerVerify, /verify_public_header_projection_set\( self::configured_public_header_languages|verify_pre_enrollment_public_header_navigation\( self::configured_public_header_languages/, "neither forward nor rollback verification may regain one all-language self-fetch");
...
assert.match(headerVerify, /pre_enrollment[\s\S]*rollback_kind[\s\S]*verify_pre_enrollment_public_header_navigation[\s\S]*public_header_frontend_cache_response_set[\s\S]*public_header_navigation_integrity_for_responses/, "only receipt-bound pre-enrollment rollback may use the raw-navigation oracle; managed forward and rollback verification must use the Public Header integrity oracle");
...
assert.match(publication, /activate_public_header_projection[\s\S]*'homepage'[\s\S]*'blog_archive'/);
assert.match(publication, /start_public_header_transition_rollback[\s\S]*public_header_rollback_verification_pending[\s\S]*finalize_public_header_transition_rollback[\s\S]*rolled_back_verified/, "Public Header rollback must remain a receipt-bound, resumable transition rather than a synchronous self-fetch");
assert.match(uninstall, /devenia_workflow_public_header_transition/, "uninstall must remove the durable Public Header transition option");
...
assert.match(publicHeaderRuntime, /verify_transition_languages[\s\S]*verify_public_header_projection[\s\S]*migration_forward[\s\S]*migration_rollback[\s\S]*rollback_invalidation_pending[\s\S]*rolled_back_verified/, "schema rollback must traverse explicit per-language forward and rollback verification before terminal cleanup");
...
for (const evidence of [
	...
	"schema1_explicit_verification_rollback_terminal",
	"schema1_to_schema2_repair_explicitly_verified",
	...
	"first_enrollment_explicit_verification_terminal",
	...
	"invalidation_failure_resumed_to_forward_verified",
	"idempotent_enrollment_transition_exercised",
	"retirement_failure_explicit_rollback_verified",
	"public_header_state_machine_http_adapter_bounded",
]) {
	assert.match(publicHeaderRuntime, new RegExp(evidence));
}
assert.match(publicHeaderRuntime, /missing_activation_state_before[\s\S]*activate_public_header_projection', array\( 'timeout' => 5 \)[\s\S]*public_header_activation_receipt_missing[\s\S]*missing_activation_state_after[\s\S]*missing_activation_raw_menus_before[\s\S]*missing_activation_inventory_before[\s\S]*missing_activation_relation_before/, "missing activation ownership must preserve raw options, menu rows, global menu inventory, posts, and metadata before staging");
assert.match(publicHeaderRuntime, /\$enrolled_fixture_state[\s\S]*delete_option\( 'devenia_workflow_public_header_manifest' \)[\s\S]*Raw drift[\s\S]*foreach \( \$enrolled_fixture_state[\s\S]*\$first_enrollment_source_menu_id[\s\S]*\$stage_a/, "the isolated pre-enrollment fallback fixture must restore its exact enrolled reader authority before the later managed complete-set activation");
...
assert.doesNotMatch(publicHeaderRuntime, /refresh_public_header_projection_for_publication|sync_menu/, "runtime fixtures must not preserve a hidden content-publication menu path");

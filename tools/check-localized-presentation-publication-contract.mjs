...
assert.match(liveFrontendBatchRuntime, /DEVENIA_WORKFLOW_FRONTEND_MATRIX_B64[\s\S]*array_keys\( \$matrix \) !== \$languages[\s\S]*'https'[\s\S]*\$matrix_host !== \$host[\s\S]*fetch_frontend_cache_surfaces[\s\S]*\$fetched/, "the dev live gate may consume only an exact installed-language matrix of credential-free same-host HTTPS URLs through the real private batch transport");

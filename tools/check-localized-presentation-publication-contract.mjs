...
assert.match(publication, /fetch_frontend_cache_surfaces[\s\S]*WpOrg\\\\Requests\\\\Transport\\\\Curl[\s\S]*WpOrg\\\\Requests\\\\Capability[\s\S]*request_multiple[\s\S]*public_header_batch_result_key_mismatch[\s\S]*public_header_batch_member_failed/, "frontend cache evidence must use one complete cURL multi request and preserve exact key-set and member fail-closed response mapping");
...
assert.doesNotMatch(frontendCacheBatchMethod, /fetch_frontend_cache_surface\(|wp_(?:safe_)?remote_get\(|array_chunk\(/, "the concurrent all-language Interface must never hide a sequential or chunked fallback path");

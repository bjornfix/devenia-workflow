...
assert.match(plugin, /PUBLIC_HEADER_ORIGIN_CONCURRENCY_LIMIT = 8[\s\S]*PUBLIC_HEADER_BATCH_BUDGET_SECONDS = 75/, "the Public Header transport must own explicit site-neutral origin-concurrency and cumulative-runtime bounds");
assert.match(publication, /fetch_frontend_cache_surfaces[\s\S]*canonical_dispatch[\s\S]*origin_dispatch[\s\S]*devenia_workflow_public_header_origin_concurrency_limit[\s\S]*min\( self::PUBLIC_HEADER_ORIGIN_CONCURRENCY_LIMIT[\s\S]*array_chunk\( \$origin_dispatch, \$origin_limit, true \)[\s\S]*public_header_batch_budget_exceeded[\s\S]*deadline[\s\S]*public_header_batch_budget_exhausted[\s\S]*request_multiple[\s\S]*public_header_batch_result_key_mismatch[\s\S]*public_header_batch_member_failed/, "frontend cache evidence must use a bounded cached/origin dispatch plan and preserve exact cap, deadline, key-set, budget, and member fail-closed response mapping");

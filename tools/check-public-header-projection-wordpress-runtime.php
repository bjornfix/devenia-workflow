...
$locked_route_target_id = 0;
$locked_route_slug_before = '';
$locked_route_slug_drift = '';
$external_writer_mode = '';
$external_writer_fixture = array();
$external_writer_result = array();
$external_writer_main_connection_id = 0;
$activation_receipt_race_mode = '';
$activation_receipt_race_result = array();
$locked_pending_reorder_result = array();
$pending_race_foreign_state = array();
$enrollment_state_race_foreign_state = array();
...
try {
	...
	$run_external_writer = static function ( string $phase, array $fixture ): array {
		$worker = realpath( __DIR__ . '/public-header-relation-lock-worker.php' );
		$wp_load = realpath( ABSPATH . 'wp-load.php' );
		if ( false === $worker || false === $wp_load || ! function_exists( 'proc_open' ) ) { throw new RuntimeException( 'Separate relation-lock worker prerequisites are unavailable.' ); }
		$command = array(
			PHP_BINARY,
			$worker,
			$wp_load,
			$phase,
			(string) ( $fixture['mode'] ?? '' ),
			(string) absint( $fixture['post_id'] ?? 0 ),
			(string) ( $fixture['fixture_slug'] ?? '' ),
			(string) ( $fixture['field'] ?? '' ),
			(string) ( $fixture['expected'] ?? '' ),
			(string) ( $fixture['replacement'] ?? '' ),
		);
		...
		if ( ! is_resource( $process ) || 3 !== count( $pipes ) ) { throw new RuntimeException( 'Separate relation-lock worker process could not start.' ); }
		fclose( $pipes[0] );
		...
		$expected_keys = array( 'success', 'phase', 'errno', 'connection_id', 'engine' );
		$exact_json = is_array( $decoded ) ? json_encode( $decoded, JSON_UNESCAPED_SLASHES ) . PHP_EOL : '';
		if ( '' !== (string) $stderr || ! is_array( $decoded ) || $expected_keys !== array_keys( $decoded ) || $exact_json !== $stdout || ( ! empty( $decoded['success'] ) ? 0 : 1 ) !== $exit_code || $phase !== (string) ( $decoded['phase'] ?? '' ) || absint( $decoded['connection_id'] ?? 0 ) < 1 || 'INNODB' !== (string) ( $decoded['engine'] ?? '' ) ) {
			throw new RuntimeException( 'Separate relation-lock worker violated its exact process/JSON contract.' );
		}
		return $decoded;
	};
	...
	$source_docs = $create_page( 'Source docs ' . $token, 'source-docs-' . $token );
	...
	$reorder_raw_pending = static function ( array $raw_pending ): array {
		if ( count( $raw_pending ) < 2 || empty( $raw_pending['items'][0] ) || ! is_array( $raw_pending['items'][0] ) || count( $raw_pending['items'][0] ) < 2 ) { throw new RuntimeException( 'Raw pending key-order fixture requires top-level and nested associative authority.' ); }
		$raw_pending['items'][0] = array_reverse( $raw_pending['items'][0], true );
		return array_reverse( $raw_pending, true );
	};
	$pending_boundary = static function () use ( &$failure_mode, &$failure_injected, &$staged_race_menu_id, &$locked_pending_reorder_result, &$pending_race_foreign_state, &$enrollment_state_race_foreign_state, $reorder_raw_pending, $call ): void {
		...
		if ( 'pending_race' === $failure_mode ) {
			$pending_race_foreign_state = array( 'race' => wp_generate_uuid4() );
			update_option( 'devenia_workflow_pending_public_header_manifest', $pending_race_foreign_state, false );
			if ( $pending_race_foreign_state !== get_option( 'devenia_workflow_pending_public_header_manifest', array() ) ) { throw new RuntimeException( 'Pending manifest race injection was not stored at the transaction boundary.' ); }
			$failure_injected = true;
			return;
		}
		if ( 'enrollment_state_race' === $failure_mode ) {
			$enrollment_state_race_foreign_state = array( 'race' => wp_generate_uuid4() );
			update_option( 'devenia_workflow_public_header_enrollment', $enrollment_state_race_foreign_state, false );
			if ( $enrollment_state_race_foreign_state !== get_option( 'devenia_workflow_public_header_enrollment', array() ) ) { throw new RuntimeException( 'Enrollment state race injection was not stored at the transaction boundary.' ); }
			$failure_injected = true;
			return;
		}
		if ( 'pending_key_reorder' === $failure_mode ) {
			$raw_pending = get_option( 'devenia_workflow_pending_public_header_manifest', array() );
			if ( ! is_array( $raw_pending ) ) { throw new RuntimeException( 'Locked raw pending key-order fixture is not an array.' ); }
			$reordered = $reorder_raw_pending( $raw_pending );
			if ( maybe_serialize( $raw_pending ) === maybe_serialize( $reordered ) || $call( 'translation_job_canonicalize', $raw_pending ) !== $call( 'translation_job_canonicalize', $reordered ) ) { throw new RuntimeException( 'Locked raw pending key-order fixture was not serialization-distinct but canonically equal.' ); }
			update_option( 'devenia_workflow_pending_public_header_manifest', $reordered, false );
			$stored = get_option( 'devenia_workflow_pending_public_header_manifest', array() );
			if ( $stored !== $reordered ) { throw new RuntimeException( 'Locked raw pending key-order fixture was not stored exactly.' ); }
			$locked_pending_reorder_result = array( 'success' => true, 'before' => $raw_pending, 'pending' => $reordered );
			$failure_injected = true;
		}
	};
	...
	$authority_relation_boundary = static function ( array $pending, array $staged ) use ( &$authority_relation_race_mode, &$authority_relation_race_translation_id, &$staged_receipt_surface_exact, &$staged_receipt_surface_observations, $call, $source_language, $source_docs, $translated ): void {
		...
		foreach ( $staged as $language => $projection ) {
			$receipt_relations = (array) ( $pending['relation_receipts'][ $language ]['relations'] ?? array() );
			...
			foreach ( $receipt_relations as $source_item_id => $relation ) {
				...
				$actual = array( 'type' => $menu_item instanceof WP_Post ? (string) $menu_item->type : '', 'object_id' => $menu_item instanceof WP_Post ? absint( $menu_item->object_id ?? 0 ) : 0, 'url' => $menu_item instanceof WP_Post ? $call( 'normalize_primary_navigation_url', (string) get_post_meta( (int) $menu_item->ID, '_menu_item_url', true ) ) : '' );
				if ( 'page' === $type ) { $matches = $matches && 'post_type' === $actual['type'] && absint( $relation['object_id'] ?? 0 ) === $actual['object_id']; }
				elseif ( 'custom' === $type ) {
					$matches = $matches && 'custom' === $actual['type'] && $call( 'normalize_primary_navigation_url', (string) ( $relation['url'] ?? '' ) ) === $actual['url'];
					if ( 'internal' === sanitize_key( (string) ( $relation['scope'] ?? '' ) ) ) {
						$expected_source_url = $call( 'normalize_primary_navigation_url', (string) get_permalink( $source_docs ) );
						$expected_target_id = $source_language === (string) $language ? $source_docs : absint( $translated[ $language ]['docs'] ?? 0 );
						$expected_target_url = $call( 'normalize_primary_navigation_url', (string) get_permalink( $expected_target_id ) );
						$matches = $matches
							&& $source_docs === absint( $relation['source_post_id'] ?? 0 )
							&& $expected_target_id === absint( $relation['target_post_id'] ?? 0 )
							&& $expected_source_url === (string) ( $relation['source_url'] ?? '' )
							&& $expected_target_url === (string) ( $relation['target_url'] ?? '' )
							&& $expected_target_url === (string) ( $relation['url'] ?? '' )
							&& '' !== (string) ( $relation['route_revision'] ?? '' );
					}
				}
				else { $matches = false; }
	...
	add_action( 'devenia_workflow_public_header_relation_before_final_revalidation', $authority_relation_boundary, 10, 2 ); $filters[] = array( 'devenia_workflow_public_header_relation_before_final_revalidation', $authority_relation_boundary, 10 );
	$locked_authority_boundary = static function ( array $pending, array $authority_lock ) use ( &$locked_authority_race_mode, &$locked_authority_race_injected, &$locked_authority_race_menu_id, &$locked_authority_race_extra_translation_id, &$locked_authority_race_source_id, &$locked_authority_race_language, &$locked_route_target_id, &$locked_route_slug_drift, &$external_writer_mode, &$external_writer_fixture, &$external_writer_result, $run_external_writer ): void {
		if ( '' !== $external_writer_mode ) {
			$external_post_id = absint( $external_writer_fixture['post_id'] ?? 0 );
			$post_locked = $external_post_id > 0 && in_array( $external_post_id, array_map( 'intval', (array) ( $authority_lock['post_ids'] ?? array() ) ), true );
			$complete_lock = 'post_slug' === $external_writer_mode
				? $post_locked && ! empty( $authority_lock['canonical_route_predicate_locked'] ) && in_array( $external_post_id, array_map( 'intval', (array) ( $authority_lock['route_post_ids'] ?? array() ) ), true )
				: $post_locked && 'identity_meta' === $external_writer_mode && ! empty( $authority_lock['canonical_meta_predicate_locked'] );
			if ( ! $complete_lock ) { throw new RuntimeException( 'Separate writer reached an incomplete relation-authority lock surface.' ); }
			$external_writer_result = $run_external_writer( 'under', $external_writer_fixture );
			if ( ! empty( $external_writer_result['success'] ) || 1205 !== absint( $external_writer_result['errno'] ?? 0 ) ) { throw new RuntimeException( 'Separate writer was not blocked by the owned InnoDB authority lock with the exact lock-wait timeout.' ); }
			return;
		}
		...
		if ( 'route_post' === $locked_authority_race_mode ) {
			if ( $locked_route_target_id < 1 || '' === $locked_route_slug_drift || empty( $authority_lock['canonical_route_predicate_locked'] ) || ! in_array( $locked_route_target_id, array_map( 'intval', (array) ( $authority_lock['post_ids'] ?? array() ) ), true ) || ! in_array( $locked_route_target_id, array_map( 'intval', (array) ( $authority_lock['route_post_ids'] ?? array() ) ), true ) ) { throw new RuntimeException( 'Internal custom-link route fixture did not receive complete route authority locks.' ); }
			global $wpdb;
			$changed = $wpdb->update( $wpdb->posts, array( 'post_name' => $locked_route_slug_drift ), array( 'ID' => $locked_route_target_id ), array( '%s' ), array( '%d' ) ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching -- Same-transaction drift proves final canonical route revalidation and rollback.
			if ( false === $changed ) { throw new RuntimeException( 'Internal custom-link route drift injection failed.' ); }
			clean_post_cache( $locked_route_target_id );
			$locked_authority_race_injected = true;
		}
	};
	...
	$manifest_items = static function ( string $prefix ) use ( $source_home, $source_blog, $source_docs, $editorial_labels ): array { return array(
		...
		array( 'source_item_id' => 800004, 'type' => 'custom', 'title' => $prefix . ' short docs', 'labels' => $editorial_labels( $prefix, 'docs' ), 'url' => (string) get_permalink( $source_docs ), 'parent_source_item_id' => 800001, 'position' => 3 ),
		array( 'source_item_id' => 800002, 'type' => 'page', 'title' => $prefix . ' short blog', 'labels' => $editorial_labels( $prefix, 'blog' ), 'object_id' => $source_blog, 'parent_source_item_id' => 0, 'position' => 4 ),
	); };
	...
	foreach ( $all_languages as $language ) {
		...
		$labels_docs = $editorial_labels( 'Active', 'docs' );
		$labels_blog = $editorial_labels( 'Active', 'blog' );
		$object_home = $source_language === $language ? $source_home : (int) $translated[ $language ]['home'];
		$object_docs = $source_language === $language ? $source_docs : (int) $translated[ $language ]['docs'];
		$object_blog = $source_language === $language ? $source_blog : (int) $translated[ $language ]['blog'];
		$create_legacy_menu = static function ( string $name, bool $managed ) use ( &$menus, $language, $labels_home, $labels_help, $labels_docs, $labels_blog, $object_home, $object_docs, $object_blog, $legacy_revision ): int {
			...
			$docs_item = wp_update_nav_menu_item( (int) $menu_id, 0, array( 'menu-item-title' => $labels_docs[ $language ], 'menu-item-url' => (string) get_permalink( $object_docs ), 'menu-item-type' => 'custom', 'menu-item-status' => 'publish', 'menu-item-parent-id' => (int) $home_item, 'menu-item-position' => 3 ) );
			$blog_item = wp_update_nav_menu_item( (int) $menu_id, 0, array( 'menu-item-title' => $labels_blog[ $language ], 'menu-item-object' => 'page', 'menu-item-object-id' => $object_blog, 'menu-item-type' => 'post_type', 'menu-item-status' => 'publish', 'menu-item-position' => 4 ) );
			if ( is_wp_error( $home_item ) || is_wp_error( $help_item ) || is_wp_error( $docs_item ) || is_wp_error( $blog_item ) ) { throw new RuntimeException( 'Could not build legacy editorial menu.' ); }
			if ( $managed ) {
				...
				foreach ( array( (int) $home_item => 800001, (int) $help_item => 800003, (int) $docs_item => 800004, (int) $blog_item => 800002 ) as $item_id => $source_item_id ) { update_post_meta( $item_id, '_devenia_translation_source_menu_item_id', $source_item_id ); }
			}
			return (int) $menu_id;
		};
	...
	$raw_fixture_menu_surface = static function () use ( &$menus, $call ): array {
		global $wpdb;
		$menu_ids = array_values( array_unique( array_filter( array_map( 'absint', $menus ) ) ) );
		sort( $menu_ids, SORT_NUMERIC );
		if ( empty( $menu_ids ) ) { return array(); }
		$menu_placeholders = implode( ', ', array_fill( 0, count( $menu_ids ), '%d' ) );
		$terms = $wpdb->get_results( $wpdb->prepare( "SELECT t.*, tt.term_taxonomy_id, tt.taxonomy, tt.description, tt.parent, tt.count FROM {$wpdb->terms} t INNER JOIN {$wpdb->term_taxonomy} tt ON tt.term_id = t.term_id WHERE t.term_id IN ({$menu_placeholders}) AND tt.taxonomy = %s ORDER BY t.term_id ASC, tt.term_taxonomy_id ASC", array_merge( $menu_ids, array( 'nav_menu' ) ) ), ARRAY_A ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared,WordPress.DB.PreparedSQL.NotPrepared,WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching -- Generated integer placeholders only; explicit fixture inventory includes authority menus whose configured names do not contain the token.
		$taxonomy_ids = array_map( 'absint', array_column( is_array( $terms ) ? $terms : array(), 'term_taxonomy_id' ) );
		$relationships = array(); $posts = array(); $postmeta = array(); $termmeta = array();
		if ( ! empty( $taxonomy_ids ) ) {
			$placeholders = implode( ', ', array_fill( 0, count( $taxonomy_ids ), '%d' ) );
			$relationships = $wpdb->get_results( $wpdb->prepare( "SELECT object_id, term_taxonomy_id, term_order FROM {$wpdb->term_relationships} WHERE term_taxonomy_id IN ({$placeholders}) ORDER BY term_taxonomy_id ASC, term_order ASC, object_id ASC", $taxonomy_ids ), ARRAY_A ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared,WordPress.DB.PreparedSQL.NotPrepared,WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching -- Generated integer placeholders only; raw exact oracle.
			$post_ids = array_values( array_unique( array_map( 'absint', array_column( is_array( $relationships ) ? $relationships : array(), 'object_id' ) ) ) );
			if ( ! empty( $post_ids ) ) {
				$post_placeholders = implode( ', ', array_fill( 0, count( $post_ids ), '%d' ) );
				$posts = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$wpdb->posts} WHERE ID IN ({$post_placeholders}) ORDER BY ID ASC", $post_ids ), ARRAY_A ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared,WordPress.DB.PreparedSQL.NotPrepared,WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching -- Generated integer placeholders only; all persisted menu-item post columns are authority.
				$postmeta = $wpdb->get_results( $wpdb->prepare( "SELECT meta_id, post_id, meta_key, meta_value FROM {$wpdb->postmeta} WHERE post_id IN ({$post_placeholders}) ORDER BY post_id ASC, meta_key ASC, meta_id ASC", $post_ids ), ARRAY_A ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared,WordPress.DB.PreparedSQL.NotPrepared,WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching -- Generated integer placeholders only; exact raw menu-item metadata oracle.
			}
		}
		if ( ! empty( $menu_ids ) ) {
			$termmeta = $wpdb->get_results( $wpdb->prepare( "SELECT meta_id, term_id, meta_key, meta_value FROM {$wpdb->termmeta} WHERE term_id IN ({$menu_placeholders}) ORDER BY term_id ASC, meta_key ASC, meta_id ASC", $menu_ids ), ARRAY_A ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared,WordPress.DB.PreparedSQL.NotPrepared,WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching -- Generated integer placeholders only; exact raw managed-menu authority.
		}
		return $call( 'translation_job_canonicalize', array( 'terms' => $terms, 'relationships' => $relationships, 'posts' => $posts, 'postmeta' => $postmeta, 'termmeta' => $termmeta ) );
	};
	$raw_nav_menu_inventory = static function (): array {
		global $wpdb;
		return (array) $wpdb->get_results( $wpdb->prepare( "SELECT t.term_id, tt.term_taxonomy_id FROM {$wpdb->terms} t INNER JOIN {$wpdb->term_taxonomy} tt ON tt.term_id = t.term_id WHERE tt.taxonomy = %s ORDER BY t.term_id ASC, tt.term_taxonomy_id ASC", 'nav_menu' ), ARRAY_A ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching -- Exact raw inventory detects orphan staged menus outside the known fixture ID set.
	};
	$raw_relation_post_surface = static function ( $post_ids ) use ( $call ): array {
		global $wpdb;
		...
		$post = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$wpdb->posts} WHERE ID IN ({$placeholders}) ORDER BY ID ASC", $post_ids ), ARRAY_A ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared,WordPress.DB.PreparedSQL.NotPrepared,WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching -- Generated integer placeholders only; exact raw post-row lock oracle.
		$meta = $wpdb->get_results( $wpdb->prepare( "SELECT meta_id, post_id, meta_key, meta_value FROM {$wpdb->postmeta} WHERE post_id IN ({$placeholders}) ORDER BY post_id ASC, meta_key ASC, meta_id ASC", $post_ids ), ARRAY_A ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared,WordPress.DB.PreparedSQL.NotPrepared,WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching -- Generated integer placeholders only; exact raw predicate-lock oracle.
		return $call( 'translation_job_canonicalize', array( 'post' => $post, 'meta' => $meta ) );
	};
	$activation_receipt_race = static function ( string $receipt, array $pending ) use ( &$activation_receipt_race_mode, &$activation_receipt_race_result, $call, $manifest_items, $reorder_raw_pending ): void {
		if ( ! in_array( $activation_receipt_race_mode, array( 'replace_pending', 'raw_normalization_equivalent', 'raw_key_reorder' ), true ) ) { return; }
		if ( 'raw_key_reorder' === $activation_receipt_race_mode ) {
			$activation_receipt_race_mode = '';
			$raw_pending = get_option( 'devenia_workflow_pending_public_header_manifest', array() );
			if ( ! is_array( $raw_pending ) ) { throw new RuntimeException( 'Pre-staging raw pending key-order fixture is not an array.' ); }
			$reordered = $reorder_raw_pending( $raw_pending );
			if ( maybe_serialize( $raw_pending ) === maybe_serialize( $reordered ) || $call( 'translation_job_canonicalize', $raw_pending ) !== $call( 'translation_job_canonicalize', $reordered ) ) { throw new RuntimeException( 'Pre-staging raw pending key-order fixture was not serialization-distinct but canonically equal.' ); }
			update_option( 'devenia_workflow_pending_public_header_manifest', $reordered, false );
			$stored = get_option( 'devenia_workflow_pending_public_header_manifest', array() );
			if ( $stored !== $reordered ) { throw new RuntimeException( 'Pre-staging raw pending key-order fixture was not stored exactly.' ); }
			$activation_receipt_race_result = array( 'success' => true, 'before' => $raw_pending, 'pending' => $reordered );
			return;
		}
		if ( 'raw_normalization_equivalent' === $activation_receipt_race_mode ) {
			$activation_receipt_race_mode = '';
			$raw_pending = get_option( 'devenia_workflow_pending_public_header_manifest', array() );
			if ( ! is_array( $raw_pending ) || empty( $raw_pending['items'][0] ) || ! is_array( $raw_pending['items'][0] ) ) { throw new RuntimeException( 'Raw pending receipt-race fixture has no canonical first item.' ); }
			$normalized_before = $call( 'normalize_public_header_manifest', $raw_pending );
			$raw_pending['items'][0]['receipt_race_discarded_field'] = 'independent-raw-owner';
			update_option( 'devenia_workflow_pending_public_header_manifest', $raw_pending, false );
			$normalized_after = $call( 'normalize_public_header_manifest', $raw_pending );
			if ( $call( 'translation_job_canonicalize', $normalized_before ) !== $call( 'translation_job_canonicalize', $normalized_after ) || $call( 'translation_job_canonicalize', $raw_pending ) === $call( 'translation_job_canonicalize', $normalized_after ) ) { throw new RuntimeException( 'Raw pending receipt-race fixture was not normalization-equivalent but byte-distinct.' ); }
			$activation_receipt_race_result = array( 'success' => true, 'pending' => $raw_pending, 'normalized' => $normalized_after );
			return;
		}
		$activation_receipt_race_mode = '';
		$activation_receipt_race_result = $call( 'update_public_header_manifest', array( 'items' => $manifest_items( 'Concurrent receipt owner' ) ) );
		if ( empty( $activation_receipt_race_result['success'] ) || empty( $activation_receipt_race_result['pending'] ) || '' === (string) ( $activation_receipt_race_result['activation_receipt'] ?? '' ) ) {
			throw new RuntimeException( 'Concurrent pending replacement fixture could not stage its independently owned manifest.' );
		}
	};
	add_action( 'devenia_workflow_public_header_before_activation_receipt_revalidation', $activation_receipt_race, 10, 2 ); $filters[] = array( 'devenia_workflow_public_header_before_activation_receipt_revalidation', $activation_receipt_race, 10 );
	...
	if ( empty( $migration_stage['staged'] ) || '' === $migration_activation_receipt ) { throw new RuntimeException( 'Schema-2 migration draft could not be staged with its exact activation receipt: ' . wp_json_encode( array( 'migration_stage' => $migration_stage, 'migration_draft' => $migration_draft, 'state_before' => $migration_stage_state_before, 'state_after' => $migration_stage_state_after, 'retained_authority_menu_ids' => $retained_authority_menu_ids, 'conflicting_authority_menu_id' => $conflicting_authority_menu_id, 'conflicting_menu_exists_after_delete' => (bool) wp_get_nav_menu_object( $conflicting_authority_menu_id ) ) ) ); }
	$migration_pending = get_option( 'devenia_workflow_pending_public_header_manifest', array() );
	$missing_relation_receipts_pending = $migration_pending;
	unset( $missing_relation_receipts_pending['relation_receipts'] );
	update_option( 'devenia_workflow_pending_public_header_manifest', $missing_relation_receipts_pending, false );
	$missing_relation_state_before = array( 'manifest' => get_option( 'devenia_workflow_public_header_manifest', '__workflow_missing__' ), 'pending' => get_option( 'devenia_workflow_pending_public_header_manifest', '__workflow_missing__' ), 'identities' => get_option( 'devenia_workflow_localized_menu_identities', '__workflow_missing__' ), 'enrollment' => get_option( 'devenia_workflow_public_header_enrollment', '__workflow_missing__' ) );
	$missing_relation_menus_before = $managed_fixture_menu_ids();
	$missing_relation_raw_menu_before = $raw_fixture_menu_surface();
	$missing_relation_menu_inventory_before = $raw_nav_menu_inventory();
	$missing_relation_attempt = $call( 'validate_public_header_relation_receipts', $missing_relation_receipts_pending );
	$missing_relation_state_after = array( 'manifest' => get_option( 'devenia_workflow_public_header_manifest', '__workflow_missing__' ), 'pending' => get_option( 'devenia_workflow_pending_public_header_manifest', '__workflow_missing__' ), 'identities' => get_option( 'devenia_workflow_localized_menu_identities', '__workflow_missing__' ), 'enrollment' => get_option( 'devenia_workflow_public_header_enrollment', '__workflow_missing__' ) );
	if ( 'public_header_relation_receipts_missing' !== (string) ( $missing_relation_attempt['code'] ?? '' ) || array_key_exists( 'projection', $missing_relation_attempt ) || array_key_exists( 'projections', $missing_relation_attempt ) || array_key_exists( 'activation', $missing_relation_attempt ) || array_key_exists( 'cleanup', $missing_relation_attempt ) || $missing_relation_state_before !== $missing_relation_state_after || $missing_relation_menus_before !== $managed_fixture_menu_ids() || $missing_relation_raw_menu_before !== $raw_fixture_menu_surface() || $missing_relation_menu_inventory_before !== $raw_nav_menu_inventory() ) { throw new RuntimeException( 'Missing mandatory relation receipts did not stop full synchronization before staging with exact raw state: ' . wp_json_encode( $missing_relation_attempt ) ); }
	update_option( 'devenia_workflow_pending_public_header_manifest', $migration_pending, false );
	$missing_relation_restored = get_option( 'devenia_workflow_pending_public_header_manifest', array() );
	if ( $call( 'translation_job_canonicalize', $migration_pending ) !== $call( 'translation_job_canonicalize', $missing_relation_restored ) || empty( $call( 'validate_public_header_relation_receipts', $missing_relation_restored )['success'] ) ) { throw new RuntimeException( 'Missing-receipt fixture cleanup did not restore the exact valid pending manifest.' ); }
	...
	$source_meta_identity_validation = $call( 'validate_public_header_relation_receipts', $migration_pending );
	if ( ! empty( $source_meta_identity_validation['success'] ) || false === strpos( wp_json_encode( $source_meta_identity_validation ), 'relation_source_translation_identity_present' ) ) { throw new RuntimeException( 'Canonical source translation metadata did not invalidate Public Header relation authority: ' . wp_json_encode( $source_meta_identity_validation ) ); }
	...
	$source_index_identity_validation = $call( 'validate_public_header_relation_receipts', $migration_pending );
	if ( ! empty( $source_index_identity_validation['success'] ) || 'public_header_relation_authority_changed' !== (string) ( $source_index_identity_validation['code'] ?? '' ) || false === strpos( wp_json_encode( $source_index_identity_validation ), 'source_page_classified_as_translation' ) ) { throw new RuntimeException( 'An index-only source translation identity was not rejected as a read-model disagreement: ' . wp_json_encode( $source_index_identity_validation ) ); }
	$call( 'sync_translation_index_row', $source_home );
	if ( empty( $call( 'validate_public_header_relation_receipts', $migration_pending )['success'] ) ) { throw new RuntimeException( 'Source identity fixture cleanup did not restore exact authority.' ); }
	...
	// A real internal custom link binds source/target URLs and its complete route
	// revision. Drift the locked target slug in the owned transaction; final
	// relation revalidation must fail and transaction rollback must restore it.
	$locked_route_target_id = (int) $translated[ $targets[0] ]['docs'];
	$locked_route_slug_before = (string) get_post_field( 'post_name', $locked_route_target_id, 'raw' );
	$locked_route_slug_drift = $locked_route_slug_before . '-drift';
	$target_relation_rows = (array) ( $migration_pending['relation_receipts'][ $targets[0] ]['relations'] ?? array() );
	$target_internal_relation = (array) ( $target_relation_rows[800004] ?? array() );
	$expected_source_docs_url = $call( 'normalize_primary_navigation_url', (string) get_permalink( $source_docs ) );
	$expected_target_docs_url = $call( 'normalize_primary_navigation_url', (string) get_permalink( $locked_route_target_id ) );
	if ( 'internal' !== (string) ( $target_internal_relation['scope'] ?? '' ) || $source_docs !== absint( $target_internal_relation['source_post_id'] ?? 0 ) || $locked_route_target_id !== absint( $target_internal_relation['target_post_id'] ?? 0 ) || $expected_source_docs_url !== (string) ( $target_internal_relation['source_url'] ?? '' ) || $expected_target_docs_url !== (string) ( $target_internal_relation['target_url'] ?? '' ) || $expected_target_docs_url !== (string) ( $target_internal_relation['url'] ?? '' ) || '' === (string) ( $target_internal_relation['route_revision'] ?? '' ) ) { throw new RuntimeException( 'Internal custom-link receipt did not bind exact canonical source/target URL authority.' ); }
	$locked_route_state_before = array( 'manifest' => get_option( 'devenia_workflow_public_header_manifest', '__workflow_missing__' ), 'pending' => get_option( 'devenia_workflow_pending_public_header_manifest', '__workflow_missing__' ), 'identities' => get_option( 'devenia_workflow_localized_menu_identities', '__workflow_missing__' ), 'enrollment' => get_option( 'devenia_workflow_public_header_enrollment', '__workflow_missing__' ) );
	$locked_route_menus_before = $managed_fixture_menu_ids();
	...
	// Prove the real InnoDB locks from disposable PHP processes with their own
	// database connections. Each writer succeeds and restores before the owned
	// transaction, times out while the exact surface is locked, then succeeds
	// and restores twice after rollback. The second after pass proves cleanup is
	// idempotent rather than merely lucky.
	$external_writer_proofs = array();
	$source_docs_slug = (string) get_post_field( 'post_name', $source_docs, 'raw' );
	$external_writer_fixtures = array(
		'post_slug' => array( 'mode' => 'post_slug', 'post_id' => $locked_route_target_id, 'fixture_slug' => $locked_route_slug_before, 'field' => 'post_name', 'expected' => $locked_route_slug_before, 'replacement' => $locked_route_slug_before . '-writer' ),
		'identity_meta_source' => array( 'mode' => 'identity_meta', 'post_id' => $source_docs, 'fixture_slug' => $source_docs_slug, 'field' => '_devenia_translation_source_id', 'expected' => 'absent', 'replacement' => (string) $source_docs ),
		'identity_meta_language' => array( 'mode' => 'identity_meta', 'post_id' => $source_docs, 'fixture_slug' => $source_docs_slug, 'field' => '_devenia_translation_language', 'expected' => 'absent', 'replacement' => (string) $targets[0] ),
	);
	foreach ( $external_writer_fixtures as $writer_mode => $writer_fixture ) {
		global $wpdb;
		$external_writer_main_connection_id = (int) $wpdb->get_var( 'SELECT CONNECTION_ID()' ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching -- Distinct-connection behavioral lock proof.
		if ( $external_writer_main_connection_id < 1 ) { throw new RuntimeException( 'Main relation-lock oracle connection ID is unavailable.' ); }
		$writer_state_before = array( 'manifest' => get_option( 'devenia_workflow_public_header_manifest', '__workflow_missing__' ), 'pending' => get_option( 'devenia_workflow_pending_public_header_manifest', '__workflow_missing__' ), 'identities' => get_option( 'devenia_workflow_localized_menu_identities', '__workflow_missing__' ), 'enrollment' => get_option( 'devenia_workflow_public_header_enrollment', '__workflow_missing__' ) );
		$writer_menus_before = $managed_fixture_menu_ids();
		$writer_raw_menus_before = $raw_fixture_menu_surface();
		$writer_menu_inventory_before = $raw_nav_menu_inventory();
		$writer_fixture_post_id = absint( $writer_fixture['post_id'] ?? 0 );
		$writer_relation_surface_before = $raw_relation_post_surface( $writer_fixture_post_id );
		$writer_before = $run_external_writer( 'before', $writer_fixture );
		if ( empty( $writer_before['success'] ) || 0 !== (int) ( $writer_before['errno'] ?? -1 ) || $writer_relation_surface_before !== $raw_relation_post_surface( $writer_fixture_post_id ) ) { throw new RuntimeException( 'Separate writer could not prove its positive pre-lock write-and-restore path for ' . $writer_mode . '.' ); }

		$external_writer_mode = (string) ( $writer_fixture['mode'] ?? '' );
		$external_writer_fixture = $writer_fixture;
		$external_writer_result = array();
		$activation_commit_mode = 'rollback_confirmed';
		$activation_commit_injected = false;
		...
		try {
			...
			$external_writer_fixture = array();
			$activation_commit_mode = '';
			$writer_after = $run_external_writer( 'after', $writer_fixture );
			$writer_cleanup_repeat = $run_external_writer( 'after', $writer_fixture );
		}

		$worker_connection_ids = array_map( 'intval', array( $writer_before['connection_id'] ?? 0, $external_writer_result['connection_id'] ?? 0, $writer_after['connection_id'] ?? 0, $writer_cleanup_repeat['connection_id'] ?? 0 ) );
		$worker_connections_distinct = 4 === count( array_unique( $worker_connection_ids ) ) && empty( array_filter( $worker_connection_ids, static function ( int $connection_id ) use ( $external_writer_main_connection_id ): bool { return $connection_id < 1 || $connection_id === $external_writer_main_connection_id; } ) );
		$writer_state_after = array( 'manifest' => get_option( 'devenia_workflow_public_header_manifest', '__workflow_missing__' ), 'pending' => get_option( 'devenia_workflow_pending_public_header_manifest', '__workflow_missing__' ), 'identities' => get_option( 'devenia_workflow_localized_menu_identities', '__workflow_missing__' ), 'enrollment' => get_option( 'devenia_workflow_public_header_enrollment', '__workflow_missing__' ) );
		if (
			! $activation_commit_injected
			|| 'public_header_projection_activation_failed' !== (string) ( $writer_attempt['code'] ?? '' )
			|| 'public_header_state_commit_rolled_back' !== (string) ( $writer_attempt['activation']['code'] ?? '' )
			|| 'unapplied' !== (string) ( $writer_attempt['activation']['state_outcome'] ?? '' )
			|| empty( $writer_attempt['cleanup']['success'] )
			|| ! empty( $external_writer_result['success'] )
			|| 1205 !== (int) ( $external_writer_result['errno'] ?? 0 )
			|| empty( $writer_after['success'] )
			|| empty( $writer_cleanup_repeat['success'] )
			|| ! $worker_connections_distinct
			|| $external_writer_main_connection_id !== (int) $wpdb->get_var( 'SELECT CONNECTION_ID()' ) // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching -- Main connection must remain stable across the lock proof.
			|| $writer_state_before !== $writer_state_after
			|| $writer_menus_before !== $managed_fixture_menu_ids()
			|| $writer_raw_menus_before !== $raw_fixture_menu_surface()
			|| $writer_menu_inventory_before !== $raw_nav_menu_inventory()
			|| $writer_relation_surface_before !== $raw_relation_post_surface( $writer_fixture_post_id )
		) { throw new RuntimeException( 'Separate InnoDB writer lock oracle failed exact before/under/after state for ' . $writer_mode . ': ' . wp_json_encode( array( 'attempt' => $writer_attempt, 'before' => $writer_before, 'under' => $external_writer_result, 'after' => $writer_after, 'cleanup_repeat' => $writer_cleanup_repeat, 'main_connection_id' => $external_writer_main_connection_id ) ) ); }
		$external_writer_proofs[ $writer_mode ] = true;
	}
	...
	$expected_raw_navigation = array(
		...
		array( 'title' => 'Active short docs', 'url' => $call( 'normalize_primary_navigation_url', (string) get_permalink( $source_docs ) ) ),
	...
	$missing_activation_state_before = array( 'manifest' => get_option( 'devenia_workflow_public_header_manifest', '__workflow_missing__' ), 'pending' => get_option( 'devenia_workflow_pending_public_header_manifest', '__workflow_missing__' ), 'identities' => get_option( 'devenia_workflow_localized_menu_identities', '__workflow_missing__' ), 'enrollment' => get_option( 'devenia_workflow_public_header_enrollment', '__workflow_missing__' ) );
	...
	$stage_b = $call( 'update_public_header_manifest', array( 'items' => $manifest_items( 'Pending' ) ) );
	$stage_b_activation_receipt = (string) ( $stage_b['activation_receipt'] ?? '' );
	if ( empty( $stage_b['pending'] ) || '' === $stage_b_activation_receipt ) { throw new RuntimeException( 'Could not restage the pending manifest with its exact receipt after cancellation proof.' ); }
	...
	$existing_source_item_ids = array_values( array_filter( array_map( 'absint', array_column( $incomplete_items, 'source_item_id' ) ) ) );
	$existing_positions = array_values( array_filter( array_map( 'absint', array_column( $incomplete_items, 'position' ) ) ) );
	$missing_source_item_id = ( empty( $existing_source_item_ids ) ? 0 : max( $existing_source_item_ids ) ) + 1;
	$missing_position = ( empty( $existing_positions ) ? 0 : max( $existing_positions ) ) + 1;
	$incomplete_items[] = array( 'source_item_id' => $missing_source_item_id, 'type' => 'page', 'title' => 'Missing short target', 'labels' => $editorial_labels( 'Missing', 'target' ), 'object_id' => $missing_source_page, 'parent_source_item_id' => 0, 'position' => $missing_position );
	$missing_target_state_before = array( 'manifest' => get_option( 'devenia_workflow_public_header_manifest', '__workflow_missing__' ), 'pending' => get_option( 'devenia_workflow_pending_public_header_manifest', '__workflow_missing__' ), 'identities' => get_option( 'devenia_workflow_localized_menu_identities', '__workflow_missing__' ), 'enrollment' => get_option( 'devenia_workflow_public_header_enrollment', '__workflow_missing__' ) );
	$missing_target_menus_before = $managed_fixture_menu_ids();
	$missing_target_raw_menus_before = $raw_fixture_menu_surface();
	$missing_target_menu_inventory_before = $raw_nav_menu_inventory();
	$missing_target_relation_surface_before = $raw_relation_post_surface( $missing_source_page );
	$incomplete = $call( 'update_public_header_manifest', array( 'items' => $incomplete_items ) );
	$incomplete_relation = isset( $incomplete['fresh'] ) && is_array( $incomplete['fresh'] ) ? $incomplete['fresh'] : array();
	$incomplete_missing = array_values(
		array_filter(
			(array) ( $incomplete_relation['missing'] ?? array() ),
			static function ( $row ) use ( $missing_source_page ): bool {
				return is_array( $row ) && $missing_source_page === absint( $row['source_id'] ?? 0 ) && 'relation_translation_missing' === (string) ( $row['reason'] ?? '' );
			}
		)
	);
	$missing_target_state_after = array( 'manifest' => get_option( 'devenia_workflow_public_header_manifest', '__workflow_missing__' ), 'pending' => get_option( 'devenia_workflow_pending_public_header_manifest', '__workflow_missing__' ), 'identities' => get_option( 'devenia_workflow_localized_menu_identities', '__workflow_missing__' ), 'enrollment' => get_option( 'devenia_workflow_public_header_enrollment', '__workflow_missing__' ) );
	if (
		! empty( $incomplete['success'] )
		|| 'public_header_relation_receipt_build_failed' !== (string) ( $incomplete['code'] ?? '' )
		|| ! in_array( (string) ( $incomplete['language'] ?? '' ), $targets, true )
		|| 'public_header_page_relation_authority_incomplete' !== (string) ( $incomplete_relation['code'] ?? '' )
		|| 1 !== count( $incomplete_missing )
		|| $missing_target_state_before !== $missing_target_state_after
		|| $missing_target_menus_before !== $managed_fixture_menu_ids()
		|| $missing_target_raw_menus_before !== $raw_fixture_menu_surface()
		|| $missing_target_menu_inventory_before !== $raw_nav_menu_inventory()
		|| $missing_target_relation_surface_before !== $raw_relation_post_surface( $missing_source_page )
		|| $active_a_revision !== (string) ( get_option( 'devenia_workflow_public_header_manifest', array() )['revision'] ?? '' )
	) { throw new RuntimeException( 'Missing target translation did not fail before pending projection mutation: ' . wp_json_encode( $incomplete ) ); }
	$stale_activation_receipt = (string) ( $stage_b['activation_receipt'] ?? '' );
	$replacement_stage = $call( 'update_public_header_manifest', array( 'items' => $manifest_items( 'Independent receipt owner' ) ) );
	$stale_receipt_state_before = array( 'manifest' => get_option( 'devenia_workflow_public_header_manifest', '__workflow_missing__' ), 'pending' => get_option( 'devenia_workflow_pending_public_header_manifest', '__workflow_missing__' ), 'identities' => get_option( 'devenia_workflow_localized_menu_identities', '__workflow_missing__' ), 'enrollment' => get_option( 'devenia_workflow_public_header_enrollment', '__workflow_missing__' ) );
	...
	$stage_b = $call( 'update_public_header_manifest', array( 'items' => $manifest_items( 'Pending' ) ) );
	$concurrent_receipt_state_before = array( 'manifest' => get_option( 'devenia_workflow_public_header_manifest', '__workflow_missing__' ), 'identities' => get_option( 'devenia_workflow_localized_menu_identities', '__workflow_missing__' ), 'enrollment' => get_option( 'devenia_workflow_public_header_enrollment', '__workflow_missing__' ) );
	$concurrent_receipt_raw_menus_before = $raw_fixture_menu_surface();
	...
	if ( 'public_header_activation_receipt_mismatch' !== (string) ( $concurrent_receipt_attempt['code'] ?? '' ) || array_key_exists( 'projection', $concurrent_receipt_attempt ) || array_key_exists( 'projections', $concurrent_receipt_attempt ) || empty( $activation_receipt_race_result['success'] ) || empty( $concurrent_owner_validation['success'] ) || $call( 'translation_job_canonicalize', (array) ( $concurrent_owner_validation['manifest'] ?? array() ) ) !== $call( 'translation_job_canonicalize', is_array( $concurrent_pending ) ? $concurrent_pending : array() ) || $concurrent_receipt_state_before !== $concurrent_receipt_state_after || $concurrent_receipt_raw_menus_before !== $raw_fixture_menu_surface() || $concurrent_receipt_inventory_before !== $raw_nav_menu_inventory() || $concurrent_receipt_relation_before !== $raw_relation_post_surface( $source_docs ) ) { throw new RuntimeException( 'A concurrent pending owner was not preserved and rejected before projection staging: ' . wp_json_encode( $concurrent_receipt_attempt ) ); }
	$stage_b = $call( 'update_public_header_manifest', array( 'items' => $manifest_items( 'Pending' ) ) );
	$stage_b_activation_receipt = (string) ( $stage_b['activation_receipt'] ?? '' );
	if ( '' === $stage_b_activation_receipt ) { throw new RuntimeException( 'Could not restore the pending owner receipt after the concurrent replacement proof.' ); }
	$raw_equivalent_state_before = array( 'manifest' => get_option( 'devenia_workflow_public_header_manifest', '__workflow_missing__' ), 'identities' => get_option( 'devenia_workflow_localized_menu_identities', '__workflow_missing__' ), 'enrollment' => get_option( 'devenia_workflow_public_header_enrollment', '__workflow_missing__' ) );
	$raw_equivalent_pending_before = get_option( 'devenia_workflow_pending_public_header_manifest', '__workflow_missing__' );
	$raw_equivalent_normalized_before = $call( 'normalize_public_header_manifest', $raw_equivalent_pending_before );
	$raw_equivalent_managed_before = $managed_fixture_menu_ids();
	$raw_equivalent_raw_menus_before = $raw_fixture_menu_surface();
	...
	if ( 'public_header_activation_receipt_mismatch' !== (string) ( $raw_equivalent_attempt['code'] ?? '' ) || array_key_exists( 'projection', $raw_equivalent_attempt ) || array_key_exists( 'projections', $raw_equivalent_attempt ) || array_key_exists( 'activation', $raw_equivalent_attempt ) || empty( $activation_receipt_race_result['success'] ) || $raw_equivalent_pending_after !== ( $activation_receipt_race_result['pending'] ?? null ) || $call( 'translation_job_canonicalize', $raw_equivalent_normalized_before ) !== $call( 'translation_job_canonicalize', $raw_equivalent_normalized_after ) || $call( 'translation_job_canonicalize', $raw_equivalent_pending_after ) === $call( 'translation_job_canonicalize', $raw_equivalent_normalized_after ) || $raw_equivalent_state_before !== $raw_equivalent_state_after || $raw_equivalent_managed_before !== $managed_fixture_menu_ids() || $raw_equivalent_raw_menus_before !== $raw_fixture_menu_surface() || $raw_equivalent_inventory_before !== $raw_nav_menu_inventory() || $raw_equivalent_relations_before !== $raw_relation_post_surface( $canonical_relation_post_ids ) ) { throw new RuntimeException( 'A normalization-equivalent raw pending replacement retained stale activation authority or mutated projection state: ' . wp_json_encode( $raw_equivalent_attempt ) ); }
	$stage_b = $call( 'update_public_header_manifest', array( 'items' => $manifest_items( 'Pending' ) ) );
	$stage_b_activation_receipt = (string) ( $stage_b['activation_receipt'] ?? '' );
	$restored_raw_pending = get_option( 'devenia_workflow_pending_public_header_manifest', '__workflow_missing__' );
	$restored_normalized_pending = $call( 'normalize_public_header_manifest', $restored_raw_pending );
	if ( '' === $stage_b_activation_receipt || ! is_array( $restored_raw_pending ) || $call( 'translation_job_canonicalize', $restored_raw_pending ) !== $call( 'translation_job_canonicalize', $restored_normalized_pending ) || isset( $restored_raw_pending['items'][0]['receipt_race_discarded_field'] ) ) { throw new RuntimeException( 'Explicit operator restaging did not replace the normalization-equivalent foreign raw pending value with a fresh canonical receipt.' ); }
	$key_reorder_state_before = array( 'manifest' => get_option( 'devenia_workflow_public_header_manifest', '__workflow_missing__' ), 'identities' => get_option( 'devenia_workflow_localized_menu_identities', '__workflow_missing__' ), 'enrollment' => get_option( 'devenia_workflow_public_header_enrollment', '__workflow_missing__' ) );
	$key_reorder_pending_before = get_option( 'devenia_workflow_pending_public_header_manifest', '__workflow_missing__' );
	$key_reorder_managed_before = $managed_fixture_menu_ids();
	$key_reorder_raw_menus_before = $raw_fixture_menu_surface();
	...
	$stage_b = $call( 'update_public_header_manifest', array( 'items' => $manifest_items( 'Pending' ) ) );
	$stage_b_activation_receipt = (string) ( $stage_b['activation_receipt'] ?? '' );
	$prelocked_restored_pending = get_option( 'devenia_workflow_pending_public_header_manifest', '__workflow_missing__' );
	if ( '' === $stage_b_activation_receipt || ! is_array( $prelocked_restored_pending ) || $prelocked_restored_pending !== $call( 'normalize_public_header_manifest', $prelocked_restored_pending ) ) { throw new RuntimeException( 'Operator restaging did not replace the pre-staging reordered raw pending value exactly.' ); }
	$locked_reorder_state_before = array( 'manifest' => get_option( 'devenia_workflow_public_header_manifest', '__workflow_missing__' ), 'identities' => get_option( 'devenia_workflow_localized_menu_identities', '__workflow_missing__' ), 'enrollment' => get_option( 'devenia_workflow_public_header_enrollment', '__workflow_missing__' ) );
	$locked_reorder_pending_before = get_option( 'devenia_workflow_pending_public_header_manifest', '__workflow_missing__' );
	$locked_reorder_managed_before = $managed_fixture_menu_ids();
	$locked_reorder_raw_menus_before = $raw_fixture_menu_surface();
	...
	if ( ! $failure_injected || empty( $locked_pending_reorder_result['success'] ) || 'public_header_projection_activation_failed' !== (string) ( $locked_reorder_attempt['code'] ?? '' ) || 'public_header_activation_receipt_mismatch' !== (string) ( $locked_reorder_attempt['activation']['code'] ?? '' ) || 'pending' !== (string) ( $locked_reorder_attempt['activation']['slot'] ?? '' ) || array_key_exists( 'projection', $locked_reorder_attempt ) || array_key_exists( 'projections', $locked_reorder_attempt ) || empty( $locked_reorder_attempt['cleanup']['success'] ) || $locked_reorder_pending_after !== ( $locked_pending_reorder_result['pending'] ?? null ) || maybe_serialize( $locked_reorder_pending_before ) === maybe_serialize( $locked_reorder_pending_after ) || $call( 'translation_job_canonicalize', $locked_reorder_pending_before ) !== $call( 'translation_job_canonicalize', $locked_reorder_pending_after ) || $locked_reorder_state_before !== $locked_reorder_state_after || $locked_reorder_managed_before !== $managed_fixture_menu_ids() || $locked_reorder_raw_menus_before !== $raw_fixture_menu_surface() || $locked_reorder_inventory_before !== $raw_nav_menu_inventory() || $locked_reorder_relations_before !== $raw_relation_post_surface( $canonical_relation_post_ids ) ) { throw new RuntimeException( 'A locked-boundary top-level and nested raw key reorder retained the old exact activation receipt, left a projection, or changed foreign authority: ' . wp_json_encode( $locked_reorder_attempt ) ); }
	$stage_b = $call( 'update_public_header_manifest', array( 'items' => $manifest_items( 'Pending' ) ) );
	$stage_b_activation_receipt = (string) ( $stage_b['activation_receipt'] ?? '' );
	...
	$staged_projection_set_evidence = static function ( array $attempt, string $expected_manifest_revision ) use ( $languages, $call ): array {
		$menu_ids = array();
		$exact = count( $languages ) === count( (array) ( $attempt['projections'] ?? array() ) );
		foreach ( (array) ( $attempt['projections'] ?? array() ) as $language => $projection ) {
			...
			$current_receipt = $menu_id > 0 ? (string) $call( 'localized_menu_projection_revision', $menu_id ) : '';
			if ( ! array_key_exists( (string) $language, $languages ) || in_array( $menu_id, $menu_ids, true ) || $menu_id < 1 || ! wp_get_nav_menu_object( $menu_id ) || '' === $receipt || '' === $current_receipt || ! hash_equals( $receipt, $current_receipt ) || '1' !== (string) get_term_meta( $menu_id, Devenia_Workflow::TERM_META_MENU_MANAGED, true ) || sanitize_key( (string) $language ) !== sanitize_key( (string) get_term_meta( $menu_id, Devenia_Workflow::TERM_META_MENU_LANGUAGE, true ) ) || '' === $expected_manifest_revision || '' === $manifest_revision || ! hash_equals( $expected_manifest_revision, $manifest_revision ) || ! hash_equals( $manifest_revision, (string) get_term_meta( $menu_id, Devenia_Workflow::TERM_META_PUBLIC_HEADER_MANIFEST_REVISION, true ) ) ) {
				$exact = false;
			}
			$menu_ids[ (string) $language ] = $menu_id;
		}
		return array( 'exact' => $exact, 'menu_ids' => $menu_ids );
	};
	$inventory_term_ids = static function ( array $inventory ): array { $ids = array_values( array_unique( array_map( 'absint', array_column( $inventory, 'term_id' ) ) ) ); sort( $ids, SORT_NUMERIC ); return $ids; };
	$expected_managed_with_staged = static function ( array $managed_before, array $staged_ids ): array { $ids = array_values( array_unique( array_merge( $managed_before, $staged_ids ) ) ); sort( $ids, SORT_NUMERIC ); return $ids; };
	$pending_race_before = $production_header_state(); $managed_before_pending_race = $managed_fixture_menu_ids();
	...
	$pending_inventory_ids_after = $inventory_term_ids( $raw_nav_menu_inventory() );
	$pending_inventory_delta = array_values( array_diff( $pending_inventory_ids_after, $inventory_term_ids( $pending_race_inventory_before ) ) ); sort( $pending_inventory_delta, SORT_NUMERIC );
	$pending_expected_staged_ids = $pending_projection_ids; sort( $pending_expected_staged_ids, SORT_NUMERIC );
	if (
		! $failure_injected
		|| 'public_header_projection_activation_state_unresolved' !== (string) ( $pending_race['code'] ?? '' )
		|| 'critical' !== (string) ( $pending_race['severity'] ?? '' )
		|| 'public_header_activation_receipt_mismatch' !== (string) ( $pending_race['activation']['code'] ?? '' )
		|| 'pending' !== (string) ( $pending_race['activation']['slot'] ?? '' )
		|| $pending_race_foreign_state !== ( $pending_race_after['pending'] ?? null )
		|| ( $pending_race_before['manifest'] ?? null ) !== ( $pending_race_after['manifest'] ?? null )
		|| ( $pending_race_before['identities'] ?? null ) !== ( $pending_race_after['identities'] ?? null )
		|| ( $pending_race_before['enrollment'] ?? null ) !== ( $pending_race_after['enrollment'] ?? null )
		|| $pending_race_before !== ( $pending_race['activation']['before'] ?? null )
		|| $pending_race_after !== ( $pending_race['cleanup_authority']['current_state'] ?? null )
		|| ! empty( $pending_race['cleanup_authority']['allowed'] )
		|| 'unknown' !== (string) ( $pending_race['cleanup_authority']['state_outcome'] ?? '' )
		|| true !== ( $pending_race['cleanup_authority']['receipt_current_exact'] ?? null )
		|| false !== ( $pending_race['cleanup_authority']['before_exact'] ?? null )
		|| false !== ( $pending_race['cleanup_authority']['references_excluded'] ?? null )
		|| 'staged_projection_cleanup_not_authorized' !== (string) ( $pending_race['cleanup']['code'] ?? '' )
		|| array() !== (array) ( $pending_race['cleanup']['results'] ?? array() )
		|| empty( $pending_projection_evidence['exact'] )
		|| count( $pending_projection_ids ) !== count( $languages )
		|| $pending_expected_staged_ids !== $pending_inventory_delta
		|| $expected_managed_with_staged( $managed_before_pending_race, $pending_projection_ids ) !== $managed_fixture_menu_ids()
		|| $pending_race_raw_menus_before !== $raw_fixture_menu_surface()
		|| $pending_race_relations_before !== $raw_relation_post_surface( $canonical_relation_post_ids )
		|| array_key_exists( 'cache_invalidation', $pending_race )
		|| array_key_exists( 'verification', $pending_race )
		|| array_key_exists( 'retirement', $pending_race )
		|| $active_a_revision !== (string) ( $pending_race_after['manifest']['revision'] ?? '' )
	) { throw new RuntimeException( 'Pending manifest race was not preserved as a foreign critical state with its staged projections intact: ' . wp_json_encode( $pending_race ) ); }
	foreach ( array( 'manifest' => 'devenia_workflow_public_header_manifest', 'identities' => 'devenia_workflow_localized_menu_identities', 'pending' => 'devenia_workflow_pending_public_header_manifest', 'enrollment' => 'devenia_workflow_public_header_enrollment' ) as $slot => $option_key ) { '__devenia_workflow_option_missing__' === $pending_race_before[ $slot ] ? delete_option( $option_key ) : update_option( $option_key, $pending_race_before[ $slot ], false ); }
	$pending_race_restored = $production_header_state();
	if ( $pending_race_before !== $pending_race_restored ) { throw new RuntimeException( 'Pending manifest race fixture did not restore the exact four-slot authority before receipt-bound cleanup.' ); }
	$pending_cleanup_authority = $call( 'public_header_staged_cleanup_state_authority', $pending_race_before, '__devenia_workflow_option_missing__' === ( $pending_race_before['identities'] ?? null ) );
	...
	$cleanup_race_mode = '';
	$state_after_identity_race = $production_header_state();
	...
	$pending_state_after_cleanup = $production_header_state();
	if ( empty( $pending_race_fixture_cleanup['success'] ) || $pending_race_before !== $pending_state_after_cleanup || $managed_before_pending_race !== $managed_fixture_menu_ids() || $pending_race_raw_menus_before !== $raw_fixture_menu_surface() || $pending_race_inventory_before !== $raw_nav_menu_inventory() || $pending_race_relations_before !== $raw_relation_post_surface( $canonical_relation_post_ids ) ) { throw new RuntimeException( 'Pending race fixture could not restore its expected state and receipt-delete its now-unreferenced projections: ' . wp_json_encode( $pending_race_fixture_cleanup ) ); }
	$enrollment_state_race_before = $production_header_state(); $managed_before_enrollment_state_race = $managed_fixture_menu_ids();
	...
	$enrollment_inventory_ids_after = $inventory_term_ids( $raw_nav_menu_inventory() );
	$enrollment_inventory_delta = array_values( array_diff( $enrollment_inventory_ids_after, $inventory_term_ids( $enrollment_state_race_inventory_before ) ) ); sort( $enrollment_inventory_delta, SORT_NUMERIC );
	$enrollment_expected_staged_ids = $enrollment_projection_ids; sort( $enrollment_expected_staged_ids, SORT_NUMERIC );
	if (
		! $failure_injected
		|| 'public_header_projection_activation_state_unresolved' !== (string) ( $enrollment_state_race['code'] ?? '' )
		|| 'critical' !== (string) ( $enrollment_state_race['severity'] ?? '' )
		|| 'public_header_state_changed' !== (string) ( $enrollment_state_race['activation']['code'] ?? '' )
		|| 'enrollment' !== (string) ( $enrollment_state_race['activation']['slot'] ?? '' )
		|| $enrollment_state_race_foreign_state !== ( $enrollment_state_race_after['enrollment'] ?? null )
		|| ( $enrollment_state_race_before['manifest'] ?? null ) !== ( $enrollment_state_race_after['manifest'] ?? null )
		|| ( $enrollment_state_race_before['pending'] ?? null ) !== ( $enrollment_state_race_after['pending'] ?? null )
		|| ( $enrollment_state_race_before['identities'] ?? null ) !== ( $enrollment_state_race_after['identities'] ?? null )
		|| $enrollment_state_race_before !== ( $enrollment_state_race['activation']['before'] ?? null )
		|| $enrollment_state_race_after !== ( $enrollment_state_race['cleanup_authority']['current_state'] ?? null )
		|| ! empty( $enrollment_state_race['cleanup_authority']['allowed'] )
		|| 'unknown' !== (string) ( $enrollment_state_race['cleanup_authority']['state_outcome'] ?? '' )
		|| true !== ( $enrollment_state_race['cleanup_authority']['receipt_current_exact'] ?? null )
		|| false !== ( $enrollment_state_race['cleanup_authority']['before_exact'] ?? null )
		|| false !== ( $enrollment_state_race['cleanup_authority']['references_excluded'] ?? null )
		|| 'staged_projection_cleanup_not_authorized' !== (string) ( $enrollment_state_race['cleanup']['code'] ?? '' )
		|| array() !== (array) ( $enrollment_state_race['cleanup']['results'] ?? array() )
		|| empty( $enrollment_projection_evidence['exact'] )
		|| count( $enrollment_projection_ids ) !== count( $languages )
		|| $enrollment_expected_staged_ids !== $enrollment_inventory_delta
		|| $expected_managed_with_staged( $managed_before_enrollment_state_race, $enrollment_projection_ids ) !== $managed_fixture_menu_ids()
		|| $enrollment_state_race_raw_menus_before !== $raw_fixture_menu_surface()
		|| $enrollment_state_race_relations_before !== $raw_relation_post_surface( $canonical_relation_post_ids )
		|| array_key_exists( 'cache_invalidation', $enrollment_state_race )
		|| array_key_exists( 'verification', $enrollment_state_race )
		|| array_key_exists( 'retirement', $enrollment_state_race )
	) { throw new RuntimeException( 'Enrollment state race did not exercise the generic locked state-CAS rejection while preserving exact foreign authority and every staged projection: ' . wp_json_encode( $enrollment_state_race ) ); }
	foreach ( array( 'manifest' => 'devenia_workflow_public_header_manifest', 'identities' => 'devenia_workflow_localized_menu_identities', 'pending' => 'devenia_workflow_pending_public_header_manifest', 'enrollment' => 'devenia_workflow_public_header_enrollment' ) as $slot => $option_key ) { '__devenia_workflow_option_missing__' === $enrollment_state_race_before[ $slot ] ? delete_option( $option_key ) : update_option( $option_key, $enrollment_state_race_before[ $slot ], false ); }
	$enrollment_state_restored = $production_header_state();
	if ( $enrollment_state_race_before !== $enrollment_state_restored ) { throw new RuntimeException( 'Enrollment state race fixture did not restore the exact four-slot authority before receipt-bound cleanup.' ); }
	$enrollment_state_cleanup_authority = $call( 'public_header_staged_cleanup_state_authority', $enrollment_state_race_before, '__devenia_workflow_option_missing__' === ( $enrollment_state_race_before['identities'] ?? null ) );
	$enrollment_state_race_cleanup = $call( 'delete_staged_public_header_projections', (array) ( $enrollment_state_race['projections'] ?? array() ), $enrollment_state_cleanup_authority );
	$enrollment_state_after_cleanup = $production_header_state();
	...
	$failure_mode = 'verification_fail'; $verification_fault_remaining = 1; $verification_fault_revision = (string) ( get_option( 'devenia_workflow_pending_public_header_manifest', array() )['revision'] ?? '' ); $verification_rollback_observations = array(); $verification_fail = $call( 'sync_public_header_projection', $activation_input( array(), $stage_b_activation_receipt ) ); $assert_rolled_back( $verification_fail, 'public_header_projection_verification_failed' );
	$failure_mode = 'verification_extra_anchor'; $verification_fault_remaining = 1; $verification_fault_revision = (string) ( get_option( 'devenia_workflow_pending_public_header_manifest', array() )['revision'] ?? '' ); $extra_anchor_fail = $call( 'sync_public_header_projection', $activation_input( array(), $stage_b_activation_receipt ) ); $assert_rolled_back( $extra_anchor_fail, 'public_header_projection_verification_failed' );
	...
	$result['concurrent_pending_replacement_rejected_before_staging'] = true;
	$result['raw_normalization_equivalent_pending_rejected_before_staging'] = true;
	...
	$result['missing_relation_receipts_failed_without_raw_state_mutation'] = true;
	...
	$result['internal_custom_route_drift_rolled_back_exactly'] = true;
	$result['relation_receipts_not_persisted_in_active_manifest'] = true;
	...
	$result['separate_connection_post_lock_blocked_writer'] = true === ( $external_writer_proofs['post_slug'] ?? false );
	$result['separate_connection_meta_predicate_lock_blocked_writer'] = true === ( $external_writer_proofs['identity_meta_source'] ?? false ) && true === ( $external_writer_proofs['identity_meta_language'] ?? false );

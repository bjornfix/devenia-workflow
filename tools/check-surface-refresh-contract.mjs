...
assert.match(runtimeSource, /\$runtime_enrollment = \$call\([\s\S]*?'enroll_public_header_from_existing_menus'[\s\S]*?\$runtime_header_activation = \$call\([\s\S]*?'activate_public_header_projection'[\s\S]*?'activation_receipt' => \$runtime_activation_receipt/);
assert.match(runtimeSource, /Could not explicitly activate and verify the complete runtime Public Header Projection/);
assert.match(runtimeSource, /\$runtime_header_activation\['verification_pending'\][\s\S]*'forward_verified'[\s\S]*\$runtime_header_verification_terminal\['finalized'\]/);

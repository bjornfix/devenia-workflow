...
assert.match(publication, /not_content_owned_without_featured_image/);
...
assert.match(publication, /\$og_matches = \$expected_id <= 0 \? \$parse_success/, "global Open Graph fallback must not become content-owned media when the Artifact has no featured image");
